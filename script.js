...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderPinned(items = []) {
    ...
    items.forEach(item => {
      ...
      if (loggedIn) {
        ...
        const unpin = document.createElement('button'); unpin.className = 'ghost small'; unpin.textContent = 'Despinear'; unpin.onclick = async () => { await supabaseClient.from('items').update({ pinned: false }).eq('id', item.id); await loadItems(); };
        const del = document.createElement('button'); del.className = 'danger'; del.textContent = 'Eliminar'; del.onclick = async () => { if (!confirm('Eliminar?')) return; await supabaseClient.from('items').delete().eq('id', item.id); await loadItems(); };
  ...
  function renderItems(items = []) {
    ...
    items.forEach(item => {
      ...
      if (loggedIn) {
        ...
        const pinBtn = document.createElement('button'); pinBtn.className = 'pin'; pinBtn.textContent = item.pinned ? 'Despinear' : 'Pin'; pinBtn.onclick = async () => { await supabaseClient.from('items').update({ pinned: !item.pinned }).eq('id', item.id); await loadItems(); };
        const del = document.createElement('button'); del.className = 'danger'; del.textContent = 'Eliminar'; del.style.marginLeft = '8px'; del.onclick = async () => { if (!confirm('Eliminar?')) return; const { error } = await supabaseClient.from('items').delete().eq('id', item.id); if (error) return alert('Delete failed: ' + error.message); await loadItems(); };
  ...
  async function loadItems() {
    const { data, error } = await supabaseClient.from('items').select('*').order('pinned',{ascending:false}).order('created_at',{ascending:false});
    if (error) { console.error('loadItems', error); if (error.message && error.message.toLowerCase().includes('permission')) return alert('Permiso denegado: ejecuta setup.sql'); return alert('Error cargando publicaciones: '+error.message); }
    const pinned = (data || []).filter(i => i.pinned);
    const normal = (data || []).filter(i => !i.pinned);
    renderPinned(pinned); renderItems(normal);
    $('#items-list') && ($('#items-list').innerHTML = (data || []).map(it => `<div style="display:flex;justify-content:space-between;padding:8px 0"><div><strong>${escapeHtml(it.title)}</strong><div style="font-size:12px;color:var(--muted)">${escapeHtml(it.username||'Anon')} • ${new Date(it.created_at).toLocaleString()}</div></div><div>${loggedIn?'<button class="ghost small" data-id="'+it.id+'">Eliminar</button>':''}</div></div>`).join(''));
    if (loggedIn) { $$('#items-list .ghost').forEach(btn => btn.addEventListener('click', async (e) => { const id = e.target.getAttribute('data-id'); if (!confirm('Eliminar?')) return; const { error } = await supabaseClient.from('items').delete().eq('id', id); if (error) return alert('Delete failed: '+error.message); await loadItems(); })); }
  }
