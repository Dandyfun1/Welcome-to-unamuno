// UNAMUNO polished final - accent theme #79c1d9
...
async function loadSiteSettings() {
  ...
  if (data) {
    ...
    if (data.hero_url) document.querySelector('.hero')?.style.setProperty("background-image", `url(${data.hero_url})`);
...
async function loadItems() {
  ...
  if (error) { console.error('loadItems', error); return alert('Error cargando publicaciones: '+error.message); }
...
function renderItems(items = []) {
  ...
  items.forEach(item => {
    ...
    if (loggedIn) {
      ...
      del.onclick = async () => { if (!confirm('Eliminar?')) return; const { error } = await supabaseClient.from('items').delete().eq('id', item.id); if (error) return alert('Delete failed: ' + error.message); await loadItems(); };
...
function renderCalendar(baseDate) {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    const today = new Date(); const dayDate = new Date(year, month, d);
    ...
    dayEl.innerHTML = `<div>${d}</div>${ev ? `<div class="event">${escapeHtml(ev.title||'')}</div>` : ''}`;
...
function openEventPopup(dateStr, ev) {
  ...
  if (loggedIn) {
    ...
    $('#edit-event-btn').onclick = async () => {
      ...
      if (!title && !note) {
        if (ev) { await supabaseClient.from('calendar_events').delete().eq('event_date', dateStr); await loadEvents(); eventPopup.classList.add('hidden'); return; }
        return;
      }
